...
func DockerClusterRun(clusterMaster, imageName, outputDir, metadataSchema string, inputs, settings, mounts []string, rmDir, quiet bool) (int, error) {
	...
	dockerArgs := []string{"ssh", clusterMaster, "docker", "run"}
	...
	dockerRun := exec.Command("docker-machine", dockerArgs...)
...
func CheckRunOutput(seed *objects.Seed, outDir, metadataSchema string, diskLimit float64) {
	...
	util.PrintUtil("outdirectory: " + outDir)
	if seed.Job.Interface.Outputs.Files != nil {
