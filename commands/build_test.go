...
func TestDockerBuild(t *testing.T) {
	cases := []struct {
		...
		/*2*/ {"../examples/addition-job/", "1.0.0", "", ".", true, ""},
		/*3*/ {"../examples/extractor/", "1.0.0", ".", "../examples/extractor/Dockerfile", true, ""},
		/*4*/ {"../examples/addition-job/", "1.0.0", "", "../examples/addition-job/Dockerfile", true, ""},
	...
	for _, c := range cases {
		_, err := DockerBuild(c.directory, c.version, "", "", c.manifest, c.dockerfile, "", false)
		success := err == nil
...
func TestSeedLabel(t *testing.T) {
	...
	for _, c := range cases {
		DockerBuild(c.directory, c.version, "", "", c.manifest, ".", "", false)
