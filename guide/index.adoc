...
[WARNING]
====
If using Windows, we strongly recommend using Seed with {wsl2-url}[WSL 2] rather than native Windows to avoid incompatibilities between environments. 
...
On Linx or Mac OS, this typically involves adding an entry to a config file.
...
[WARNING]
====
We strongly recommend using Seed with {wsl2-url}[WSL 2] rather than native Windows to avoid incompatibilities between environments. 
Algorithms must use a base Linux operating system to run in Scale.
====
